...
export default function Game() {
  ...
  const draw = useCallback((ctx: CanvasRenderingContext2D, engine: GameEngine) => {
    ...
    // Draw steam/mist in background
    ctx.fillStyle = 'rgba(255, 255, 255, 0.25)';
    for (let i = 0; i < 5; i++) {
      const steamX = ((i * 200 - scrollOffset * 0.15) % (config.canvasWidth + 150)) - 75;
      const steamY = 60 + Math.sin(Date.now() / 1200 + i) * 25;
      ctx.beginPath();
      ctx.arc(steamX, steamY, 50 + Math.sin(Date.now() / 900 + i) * 15, 0, Math.PI * 2);
      ctx.arc(steamX + 40, steamY - 15, 40, 0, Math.PI * 2);
      ctx.arc(steamX + 80, steamY, 45, 0, Math.PI * 2);
      ctx.fill();
    }
    ...
    if (data.state === GameState.PLAYING || data.state === GameState.GAMEOVER || data.state === GameState.TURBAN_TOSSED) {
      ...
      drawMullah(ctx, mullahScreenX, mullah.y, mullah.width, mullah.height, mullah);
      ...
      // Draw player
      drawPlayer(ctx, playerScreenX, player.y, player.width, player.height, player.isJumping, canAttack, player.walkFrame);
    ...
    if (data.state === GameState.START) {
      drawStartOverlay(ctx, config.canvasWidth, config.canvasHeight);
    } else if (data.state === GameState.TURBAN_TOSSED) {
      drawTurbanFlippedOverlay(ctx, config.canvasWidth, config.canvasHeight, data.score, data.celebrationTimer, confettiRef.current);
      updateConfetti(confettiRef.current, config.canvasWidth, config.canvasHeight);
    } else if (data.state === GameState.GAMEOVER) {
      drawBustedOverlay(ctx, config.canvasWidth, config.canvasHeight, data.score, data.highScore, data.bustedIntensity);
    }
  }, []);

  function drawPlayer(ctx: CanvasRenderingContext2D, x: number, y: number, width: number, height: number, isJumping: boolean, canAttack: boolean, walkFrame: number) {
  ...
  function drawMullah(ctx: CanvasRenderingContext2D, x: number, y: number, width: number, height: number, mullah: Mullah) {
    ...
    if (isLooking) {
      ...
      ctx.fillStyle = '#000';
      const eyeShift = Math.sin(Date.now() / 200) * 1.5;
      ctx.beginPath();
    ...
    if (isWarning) {
      ...
      ctx.lineWidth = 3;
      const ringSize = 30 + Math.sin(Date.now() / 100) * 5;
      ctx.beginPath();
  ...
  function drawStartOverlay(ctx: CanvasRenderingContext2D, width: number, height: number) {
    // Dark overlay
  ...
  function drawTurbanFlippedOverlay(ctx: CanvasRenderingContext2D, width: number, height: number, score: number, timer: number, confetti: typeof confettiRef.current) {
    // Spawn confetti
    ...
    // TURBAN FLIPPED text with effects
  ...
  function drawBustedOverlay(ctx: CanvasRenderingContext2D, width: number, height: number, score: number, highScore: number, intensity: number) {
    ...
    // BUSTED text - GTA style
    ctx.shadowColor = '#000';
    ctx.shadowBlur = 10;
    ctx.shadowOffsetX = 4;
    ctx.shadowOffsetY = 4;

    // Text shake based on intensity
